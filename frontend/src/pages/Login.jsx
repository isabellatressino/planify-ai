...
export default function Login() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (err?.code === "EMAIL_NOT_VERIFIED" || err?.message === "Verifique seu e-mail") {
        ...
        <div className="flex w-full gap-10  lg:items-center">
          <div className="hidden lg:flex flex-col justify-center pr-10">
            ...
            <h2 className="text-3xl sm:text-4xl lg:text-5xl text-text-titles font-display mt-4">
              Domine qualquer assunto com ajuda de IA
            </h2>
            <p className="mt-4 text-base text-text-body">
              Receba um plano personalizado, um checklist diario de tarefas e um acompanhamento simples do seu avanco.            </p>
          </div>
          <div className="mx-auto w-full max-w-md">
            <Card className="bg-surface/85 p-8 shadow-xl">
              <div className="text-center">
                <h1 className="font-display text-3xl font-semibold text-text-titles">Bem-vindo de volta</h1>
              ...
              <p className="mt-6 text-center text-sm text-text-body">
                Ainda nao tem conta?{" "}
                <Link to="/register" className="font-medium text-primary hover:text-primary-hover transition-all duration-300 ease-out cursor-pointer">
                  Cadastre-se
                </Link>
              </p>
            </Card>
